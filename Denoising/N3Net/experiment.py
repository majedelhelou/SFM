'''
This file is modified to support blind training and SFM from:
...
#             (img_dataset.PlainImageFolder(root=denoising_data.set12_val_dir, transform=transform_test, cache=True), "Set12"),
    ...
    def data_preprocessing_Blind_SFM(self, input):
        args = self.args
        
        # NOISE For Blind Training
        noise = torch.zeros_like(input)
        ...
        if args.DCT_DOR == 0:
            ...
            # SFM The INPUT
            input_SFM = np.zeros(input.size(),dtype='float32')
            dct_bool = np.random.choice([1, 0], size=(input.size()[0],), p=[args.DCT_DOR, 1-args.DCT_DOR])

            for img_idx in range(input.size()[0]):
                if dct_bool[img_idx] == 1:
                    img_numpy, mask = random_drop(input[img_idx,:,:,:].cpu().data.numpy(), mode=2, SFM_center_radius_perc=0.85, SFM_center_sigma_perc=0.15)
                    input_SFM[img_idx,0,:,:] = img_numpy

            noisy = torch.from_numpy(input_SFM).cuda() + noise
...
#         decay = 10**(-3.0*epoch/50.0)
        if epoch > 100:
            return 0
#         decay = 10**(-5.0*epoch/100.0)
        decay = 10**(-5.0*epoch/50.0)
        return decay
