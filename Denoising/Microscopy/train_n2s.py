...
from mask import Masker #used for Noise2Self
...
class Parser(argparse.ArgumentParser):
    def __init__(self):
        super(Parser, self).__init__(description='Training N2S')
        self.add_argument('--exp-name', type=str, default='n2s', help='experiment name')
...
try:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        #     sys.exit(0)
        psnr, mse = 0., 0.
        masker = Masker(mode='random') #N2S masker object
        for batch_idx, (noisy_input, noisy_target, clean) in enumerate(train_loader):
            iters += 1
            noisy_input, clean = noisy_input.to(device), clean.to(device)
            ...
            # Copy only needed for SFM with N2S
            noisy_input_copy = noisy_input.cpu().numpy()
            noisy_input_copy = torch.from_numpy(noisy_input_copy).to(device)
            ...
            # For self-supervised N2S learning:
            input_N2S, mask_N2S = masker.mask(noisy_input, batch_idx)
            
            model.zero_grad()
            denoised = model(input_N2S)
            loss = F.mse_loss(denoised*mask_N2S, noisy_input_copy*mask_N2S, reduction='sum')
            loss.backward()
