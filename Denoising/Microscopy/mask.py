...
class Masker():
    ...
    def mask(self, X, i):
        ...
        if self.mode == 'interpolate':
            ...
            masked = randomreplace_mask(X, mask, mask_inv)
        else:
...
def randomreplace_mask(tensor, mask, mask_inv):
    device = tensor.device

    mask = mask.to(device)

    # Random values from [-0.5, 0.5] for fluo N2S mask
    random_image = np.random.rand(*list(tensor.shape)) - 0.5
    filtered_random = torch.from_numpy(random_image).float().to(device)
    
    return filtered_random * mask + tensor * mask_inv
